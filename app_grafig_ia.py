...
from app import convert_to_wav, transcribe_audio, improve_transcription_with_ai
...
def main(page: ft.Page):
    ...
    # Função para criar um bloco de área de texto com botões
    def criar_bloco_texto(titulo_texto, texto_placeholder1, texto_placeholder2):
        # Título da área
        titulo = ft.Text(
            titulo_texto,
            size=16,
        ...
        botao_limpar = criar_botao(
            f"Limpar {titulo_texto.lower()}",
            ft.Icons.CLEANING_SERVICES_OUTLINED,
            COR_ALERTA,
            COR_FUNDO,
            COR_ALERTA,
            ft.BorderSide(1, COR_ALERTA),
            desativado=True  # Começa desativado até ter um texto
        )
        ...
        return titulo, area_texto, botao_limpar, botao_copiar, container_botoes
    ...
    def iniciar_transcricao(e):
        try:
            ...
            mostrar_status(
                "Melhorando o texto com IA... Por favor, aguarde.", COR_PRINCIPAL)
            try:
                texto_melhorado = improve_transcription_with_ai(
                    texto_transcrito)
                area_texto_melhorado.content.controls = [
                    ft.Text(
                        texto_melhorado,
                        size=14,
                        color=COR_TEXTO_PRINCIPAL,
                ...
                mostrar_status("Texto melhorado com sucesso!", COR_PRINCIPAL)

                # Habilitar os botões de limpar e copiar para o texto melhorado
                botao_limpar_melhorado.disabled = False
                botao_copiar_melhorado.disabled = False
            except Exception as ai_error:
                print(f"Erro ao melhorar texto: {str(ai_error)}")
                mostrar_status(
                    f"Erro ao melhorar texto: {str(ai_error)}", COR_ALERTA)
    ...
    area_upload = ft.Container(
        ...
        margin=ft.margin.symmetric(horizontal=20),
        bgcolor=COR_FUNDO,
    )    # Criando o bloco de transcrição original
    titulo_transcricao, area_transcricao, botao_limpar, botao_copiar, container_botoes_transcricao = criar_bloco_texto(
        "Transcrição",
        "Nenhuma transcrição ainda",
        "       Faça upload de um arquivo de áudio para iniciar"
    )

    # Criando o bloco de texto melhorado
    titulo_texto_melhorado, area_texto_melhorado, botao_limpar_melhorado, botao_copiar_melhorado, container_botoes_melhorado = criar_bloco_texto(
        "Texto Melhorado",
        "Nenhum texto melhorado ainda",
        "       Faça upload de um arquivo de áudio para iniciar"
    )
    # Definindo as funções para os botões de transcrição
    ...
    def limpar_transcricao(e):
        ...
        # Limpar também o texto melhorado
        area_texto_melhorado.content.controls = [
            ft.Text(
                "Nenhum texto melhorado ainda",
                italic=True,
        ...
        botao_limpar.disabled = True
        botao_copiar.disabled = True
        botao_limpar_melhorado.disabled = True
        botao_copiar_melhorado.disabled = True
        # Mostrar mensagem de limpeza
        mostrar_status("Transcrição e texto melhorado limpos.")
        page.update()

    botao_limpar.on_click = limpar_transcricao
    botao_copiar.on_click = copiar_transcricao

    # Definindo as funções para os botões de texto melhorado
    def limpar_texto_melhorado(e):
        # Restaurar o conteúdo padrão da área de texto melhorado
        area_texto_melhorado.content.controls = [
            ft.Text(
                "Nenhum texto melhorado ainda",
                italic=True,
        ...
        # Desabilitar os botões de limpar e copiar para o texto melhorado
        botao_limpar_melhorado.disabled = True
        botao_copiar_melhorado.disabled = True
        # Mostrar mensagem de limpeza
        mostrar_status("Texto melhorado limpo.")
        page.update()

    def copiar_texto_melhorado(e):
        # Verificamos se há texto para copiar
        if len(area_texto_melhorado.content.controls) == 1 and isinstance(area_texto_melhorado.content.controls[0], ft.Text):
            texto = area_texto_melhorado.content.controls[0].value
            page.set_clipboard(texto)
            mostrar_status(
                "Texto melhorado copiado para a área de transferência!", COR_SUCESSO)
        else:
            mostrar_status("Não há texto melhorado para copiar.", COR_ALERTA)

    botao_limpar_melhorado.on_click = limpar_texto_melhorado
    # Conteúdo principal com layout simplificado
    botao_copiar_melhorado.on_click = copiar_texto_melhorado
    conteudo_principal = ft.Column(
        [
            ...
            area_transcricao,
            ft.Container(padding=8),   # Espaçamento pequeno
            container_botoes_transcricao,

            # Bloco do texto melhorado
            ft.Container(
                content=titulo_texto_melhorado,
                margin=ft.margin.only(left=20, top=15)
            ),
            ft.Container(padding=8),   # Espaçamento pequeno
            area_texto_melhorado,
            ft.Container(padding=8),   # Espaçamento pequeno
            container_botoes_melhorado,
